...
def _base_canvas() -> tuple[Image.Image, ImageDraw.ImageDraw]:
    ...
    draw.text((W - 440, 19), "OpenSky → DuckDB → dbt → Streamlit  ·  🧪 Demo mode",
              fill=TEXT_SEC, font=F["sm"])
    return img, draw
...
def _metric_card(draw: ImageDraw.ImageDraw, x: int, y: int,
...
def _table_row(draw: ImageDraw.ImageDraw, y: int, cells: list[str],
               col_x: list[int], fill: tuple, text_color: tuple = TEXT_PRI) -> None:
...
def generate_overview() -> None:
    ...
    snap_hdrs  = ["Region", "Captured at (UTC)", "Aircraft detected",
                  "GPS-positioned", "On ground", "Avg ground speed (m/s)"]
    ...
    draw.text((28, y + 9), "🚨  Recent anomaly events  (40 detected)  ▸", fill=ACCENT_RED, font=F["sm"])

    # Footer
    draw.text((16, H - 22),
              "Air Traffic Pulse  ·  Analytics Engineering Portfolio Project  ·  v0.1.0",
              fill=TEXT_SEC, font=F["xs"])
...
def generate_timeseries() -> None:
    ...
    for ax, ay in anomaly_pts:
        draw.ellipse([(ax - 6, ay - 6), (ax + 6, ay + 6)],
                     fill=ACCENT_YELL, outline=BG_DARK, width=2)
    ...
    draw.ellipse([(lx + 6, cy0 + 50), (lx + 22, cy0 + 66)],
                 fill=ACCENT_YELL, outline=BG_DARK, width=1)
    ...
    draw.rounded_rectangle([(16, y), (W - 16, y + 32)], radius=4, fill=BG_CARD)
    draw.text((28, y + 9), "🚨  Anomaly buckets for 🇬🇧 London  (10 total)  ▾",
              fill=ACCENT_RED, font=F["sm"])
    y += 36
